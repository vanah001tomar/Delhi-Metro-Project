...
void displayFunctions()
{
    ...
    cout<<"                                             -1. Add Stations(Admin Only)"<<endl<<endl;
...
B:
    ...
    {
        if(src[i]<'0' or src[i]>'9') digit = false;
    }

    if(digit)
    {
        if(stoi(src)>=0 && stoi(src)<=stationIdUsed) src = m.stationFromId[stoi(src)];
    }
...
C:
    ...
    {
        if(dest[i]<'0' or dest[i]>'9') digit = false;
    }

    if(digit)
    {
        if(stoi(dest)>=0 && stoi(dest)<=stationIdUsed) dest = m.stationFromId[stoi(dest)];
    }
